...
import { useEditReferJobMutation } from "./mutations";
import {
...
interface EditReferJobDialogueProps {
...
export default function EditReferJobDialogue({
  referJob,
  open,
  onOpenChange,
}: EditReferJobDialogueProps) {
  // Edit mutation
  const mutation = useEditReferJobMutation();

  // Form
  const form = useForm<CreateReferJobSchemaType>({
    resolver: zodResolver(createReferJobSchema),
    defaultValues: {
      title: referJob.title,
      company: referJob.company,
      location: referJob.location,
      salary: referJob.salary,
      skills: referJob.skills.join(", "), // Convert array to string
      category: referJob.category.join(", "), // Convert array to string
      applyLink: referJob.applyLink,
      jobType: referJob.jobType as "onsite" | "remote" | undefined,
      workType: referJob.workType as "full-time" | "part-time" | undefined,
    },
  });

  // Handle submit
  async function onSubmit(editedValues: CreateReferJobSchemaType) {
    mutation.mutate(
      {
        referJobId: referJob.id,
        editedReferJobData: editedValues,
      },
      {
        onSuccess: () => {
          onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit Refered Job</DialogTitle>
        </DialogHeader>
        <Form {...form}>
